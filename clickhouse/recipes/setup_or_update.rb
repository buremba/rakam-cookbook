...
bash "download and worker" do
  code <<-EOH
    ...
    su webapp -l -c 'cd rakam-clickhouse-worker; mvn clean install -DskipTests && rm -rf ../worker/lib && if [ -d '../worker' ] ; then mv target/*-bundle/clickhouse-worker-*/lib ../worker/lib; else mv target/*-bundle/clickhouse-worker-*/ ../worker; fi;'
  EOH
end
