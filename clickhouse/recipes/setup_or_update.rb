...
bash "download and worker" do
  code <<-EOH
    ...
    su webapp -l -c 'cd rakam-clickhouse-worker; mvn clean install -DskipTests && rm -rf ../worker && mv rakam-clickhouse-worker/target/*-bundle/rakam-clickhouse-worker-* ../worker'
  EOH
end
